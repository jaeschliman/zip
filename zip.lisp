...
(defun %directory (d)
  ...
  #-(or allegro clisp) (cl-fad:list-directory d))
...
(defun %pathname-for-directory (f)
  #+(or allegro clisp ccl) f
  #-(or allegro clisp ccl) (make-pathname :name :wild :type :wild :defaults f)
  )
...
(defun zip (pathname source-directory &key (if-exists :error))
  (let ((base (%directory-namestring (merge-pathnames source-directory (truename *default-pathname-defaults*)))))
